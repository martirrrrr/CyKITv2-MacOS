...
import sys
sys.path.append('/Users/martinaroscica/Downloads/CyKITv2-master/cython-hidapi/')
sys.path.append('/Users/martinaroscica/Downloads/CyKITv2-master/')
...
if os.name == 'nt':  # windows
    ...
    import hidapi
    print(hidapi.__path__)
    import sys

    # Rimuove la versione sbagliata di hid dal sys.modules
    if "hid" in sys.modules:
        del sys.modules["hid"]

    # Aggiunge il percorso corretto
    sys.path.insert(0, "/Users/martinaroscica/Downloads/CyKITv2-master/cython-hidapi")

    import hid  # Ora dovrebbe caricare quello giusto
    print("hid module path:", hid.__file__)
    #import hid
    #hid_init()
...
class EEG(object):
    ...
    def start(self):
        print("Threading started enumeration process")
    ...
    def Setup(self, model, config):
        ...
        print("Cerco I devices")
        #avail_hid_devices = hid.find_all_hid_devices() if os.name == 'nt' else hid_enumerate()
        avail_hid_devices = hid.find_all_hid_devices() if os.name == 'nt' else hid.enumerate()
        print("I miei devices:")
                    ...
                    if useDevice.upper() == "Y":
                        ...
                        if threadMax < 2:
                            if os.name == 'nt':
                                ...
                                self.hid.open(device['vendor_id'],device['product_id'],serial_number=device['serial_number'])
                                #hid.hid_open(device.vendor_id, device.product_id,serial_number=device.serial_number)
                            else:
                                raise UnsupportedOSError()
